...
@Injectable()
export class SelectFormGroupBranchDateTimeService {
  ...
  private branchDateTimeText = new BehaviorSubject<string>('');
  branchDateTimeText$ = this.branchDateTimeText.asObservable();
    ...
    const control_form_group = {
      ...
      subscribe: () => {
        self.formValueChanges.subscribe((res: any) => {
          ...
          self.branchDateTimeText.next(
            self.convertArrayToTextId(res.branch_array, 'branch_date_time')
          );
        });
      },
      setup: () => {
